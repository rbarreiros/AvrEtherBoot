...
#include "config.h"
	...
	*************************************************************/
	...
	#define ENC_CS                  0		//4 für Pollin NET I/O Board
	...
	#define ETH_INTERRUPT           INT1_vect
	#define ETH_INT_ACTIVE          (!(ENC_PIN & (1<<ENC_INT)))

	
	#define ETH_INT_ENABLE EIMSK |= (1<<INT1)
	#define ETH_INT_DISABLE EIMSK &= ~(1<<INT1)
	...
	*************************************************************/
	...
	#define LO8(x) ( (unsigned char) ((x) & 0xFF) )
	#define HI8(x) ( (unsigned char) (((x) >> 8) & 0xFF) )
